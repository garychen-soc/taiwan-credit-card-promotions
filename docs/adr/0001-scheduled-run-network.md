...
此外，月排程與週排程原本先以 `generated_at` 做 30 分鐘重疊判斷，並在本輪 publish guard 被攔時禁止任何提交。兩條規則交互作用，使前一輪已通過 guard、但尚未提交的 `docs/data/promotions.json` 被當成不可處理的工作區變更，可能無限期擱置。
...
4. 在 30 分鐘重疊閘門之前，先檢查未提交的 `docs/data/promotions.json`。只有同時符合下列條件，才視為「待補提交的已通過快照」：
...
5. 待補快照先完成測試並建立獨立本機 commit。若後續新更新被 guard 攔截，只禁止提交該次被攔候選；先前已驗證的恢復 commit 仍可推送，避免再次擱置。
...
資料只能在以下條件全部成立後提交：更新退出碼為 0 或 2、`reports/latest.json` 的 guard passed、failed source 不超過 2、DNS 失敗為 0、Python tests 通過、前端 JavaScript 語法通過、JSON 與 diff 檢查通過。推送後還要確認 `Refresh and deploy GitHub Pages` 成功，並分開回報 Git 合併、Pages 部署與線上驗證狀態。
